...
# Load data on server
df_orig = pd.read_excel(r'bokeh-dash-e/result_data_x.xlsx', names=['index', 'type', 'date', 'code', \
    'filter_one', 'filter_two', 'filter_three', 'filter_four', 'recommendation', 'easiness', 'overall', 'question_one', \
    'rec_sc', 'eas_sc', 'sentiment', 'lang', 'question_one_filtered_lemmas'])
...
for type_filter in general_dict['type_list']:
    ...
    general_dict['individual_filters'][type_filter] = np.concatenate((df_orig.filter_two.unique(),df_orig.filter_three.unique(),df_orig.filter_four.unique())).tolist() + [type_filter]
...
def update_filters():
    ...
    common_filters = [general_dict['filter_3'].labels[i] for i in general_dict['filter_3'].active] + general_dict['filter_2'].value + general_dict['filter_4'].value
    for type_filter in general_dict['type_list']:
...
def filter_df(type_filter):
    ...
    general_dict['individual_filtered_df'][type_filter] = general_dict['individual_df'][type_filter].loc[(general_dict['individual_df'][type_filter]['filter_two'].isin(filter_list) & general_dict['individual_df'][type_filter]['filter_three'].isin(filter_list) & general_dict['individual_df'][type_filter]['filter_four'].isin(filter_list))].copy()
...
data_source = ColumnDataSource(pd.DataFrame(columns=['filter_two', 'filter_three', 'filter_four', 'recommendation', 'easiness']))
...
def typed_callback_c(type_filter):
    ...
    current = df_points.loc[(df_points['recommendation'].isin(recommendations)) & (df_points['easiness'].isin(easinesses)) & (df_points['filter_two'].isin(general_dict['individual_filters'][type_filter])) & (df_points['filter_three'].isin(general_dict['individual_filters'][type_filter])) & (df_points['filter_four'].isin(general_dict['individual_filters'][type_filter]))]
...
buttons_row = row(column(children=[general_dict['filter_4'], button_3, button_4], height=200, max_height=200, width=320), column(children=[general_dict['filter_2'], button_5, button_6], height=200, max_height=200, width=190), data_table)
...
# row_three = row(general_dict['individual_filters_vis'][general_dict['type_list'][0]], sizing_mode='scale_width', css_classes=['super-flex'], name='row_three')
...
def init_frequency_points(type_filter):
    ...
    filter_columns = ['filter_two', 'filter_three', 'filter_four']
